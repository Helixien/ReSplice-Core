...
namespace ReSpliceCore
{
    ...
    public class Window_SeparateGene : Window
    {
        ...
        public override void DoWindowContents(Rect inRect)
        {
            ...
            GUI.color = Color.grey;
            Text.Font = GameFont.Tiny;
            Widgets.Label(geneSeparationExplanationRect, "RS.SeparateGeneExplanation".Translate());
            Text.Font = GameFont.Small;
            GUI.color = Color.white;
            ...
            if (Widgets.ButtonText(acceptButtonRect, "RS.StartSeparating".Translate()))
            {
                if (chosenGene != null)
                {
                ...
                {
                    Messages.Message("RS.SelectAGeneToSeparate".Translate(), MessageTypeDefOf.CautionInput);
                }
            }
        }
            ...
            if (gene.biostatArc == 0)
            {
                switch (geneType)
                {
                    case GeneType.Endogene:
                        cachedTexture = (CachedTexture)VanillaExpandedFramework_GeneUIUtility_DrawGeneBasics_Patch.ChooseEndogeneBackground(gene);
                        break;
                    case GeneType.Xenogene:
                        cachedTexture = (CachedTexture)VanillaExpandedFramework_GeneUIUtility_DrawGeneBasics_Patch.ChooseXenogeneBackground(gene);
                        break;
                }
            }
